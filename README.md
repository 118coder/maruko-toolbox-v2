...
> **完整发布包**：运行 `pack_full.py` 可生成「完整版」zip——exe 旁自带全套工具链
> tools\（ffmpeg 已替换为现代 7.1 构建），用户**无需安装原版小丸**即可解压即用。
> 定位器优先级：exe 旁 tools\ → 设置页手动路径 → 注册表 → 常见路径 → PATH。首次启动会自动定位原版小丸工具箱的 `tools\` 目录（也可在「设置」页手动指定）。
